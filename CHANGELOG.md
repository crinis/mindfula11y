...
- Buttons that start an asynchronous action (generating or saving alternative text, triggering or cancelling scans) now stay focusable while the action runs; they were disabled for the request window, which dropped keyboard focus to the page. Retry/refresh buttons after failed loads also moved out of the live status regions — embedded in `role="status"` they were re-announced as status text by some screen readers — and the Missing Alternative Texts pagination no longer carries redundant `listitem` roles.
